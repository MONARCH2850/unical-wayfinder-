...
To share tagged spots from a phone without Wi-Fi, deploy the Django backend at a publicly reachable HTTPS URL and define `window.UNICAL_API_BASE` before loading `app.js` (for example, `https://api.example.com`). The **Upload Saved Spots** button sends the queued `unical_tagged_spots` records to `/api/spots/sync/` over mobile data. Saved places remain on the phone for map use; the tagged upload queue is cleared only after a successful response. Other devices automatically pull the shared records from `/api/locations/` on startup. The Django server must allow the phone's origin through CORS.
