...
To push spots saved on a phone to a Django server running on the same local network, define `window.UNICAL_PC_LOCAL_IP` before loading `app.js` (for example, `192.168.1.20`). The **Push Saved Spots to PC** button sends the queued `unical_tagged_spots` records to `http://<PC_LOCAL_IP>:8000/api/spots/sync/` and clears the queue only after a successful response. The Django server must allow the phone's origin through CORS and the network firewall must permit port 8000.
